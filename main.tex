...
\begin{figure}[t]
\centering
\includegraphics[width=0.96\columnwidth]{architecture.png}
...
\paragraph{Data and Code Availability.} The experiment uses a locally supplied 5,559-row CSV derived from the public SMS Spam Collection. The exact local CSV used for the reported experiment is not archived with this manuscript. The accompanying source package includes the experiment script, benchmark script, reported result file, confidence-interval code, plotting utilities, and dependency information needed to reproduce the documented workflow. Future releases should include an exact dataset checksum or version identifier, the original preprocessing script, package versions, and system configuration to support byte-level reproducibility.
...
These values produce a more nuanced conclusion than accuracy alone. The model achieves high performance on the observed split, but its performance is not symmetric across the two error types. In a user-facing filter, a false negative allows an unwanted message to reach the user, whereas a false positive could hide an important legitimate message. The preferred operating point would therefore depend on the intended application.
...
The experiment demonstrates that a standard bag-of-words representation combined with Multinomial Naive Bayes can achieve high predictive performance on the supplied SMS corpus. The observed 99.01\% accuracy and 95.69\% spam F1-score indicate that the baseline captures substantial lexical regularities in the data.
...
The observed performance supports the use of classical sparse text classification as a useful baseline for lightweight prototypes and reproducibility-oriented studies. It does not establish state-of-the-art performance or universal generalization. A stronger next step is a controlled multi-model comparison with repeated validation, explicit resource measurements, and analysis of false-negative message types.
